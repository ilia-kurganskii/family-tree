...
export function expandBranch(root: ProcessedFamilyNode, id: string): ProcessedFamilyNode {
    const newRoot = findBranchAndExpand({...root}, id)
    return newRoot;
}

export function findBranchAndExpand(node: ProcessedFamilyNode, id: string): ProcessedFamilyNode {
    if (!node.children) {
        return node;
    }
    ...
    if (hasChildWithId) {
        ...
        node.children = node.children.map(child => findBranchAndExpand(child, id))
    }
    return node;
